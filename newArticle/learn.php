...
$hashlist = array("c", "c++", "c#", "java", "python", "html", "css", "javascript", "php");
$hash = "";
foreach ($hashlist as $hashindex) {
    $hash .= ",";
    if (isset($_POST['learn-' . $hashindex])) {
        $hash .= "'1"."'";
    } else {
        $hash .= "'0"."'";
    }
...
function createSQL($hash, $link)
{
    ...
    $sql .= " (title,article,link,publish_date,c,`c++`,`c#`,java,python,html,css,js,php) VALUES(";
